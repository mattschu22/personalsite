...
export default function PaxosInfoPanel() {
  return (
    <>
      {/* Mobile: Always expanded panel */}
      <div className="md:hidden absolute bottom-0 left-0 right-0 z-30">
        <div className="bg-white/95 backdrop-blur-sm border-t border-slate-200 shadow-lg px-4 py-3">
          <span className="text-[11px] font-semibold text-slate-700 block mb-2">What is Paxos?</span>
          <div className="grid grid-cols-2 gap-3">
            {/* The Problem */}
            <div className="space-y-1.5">
              <div className="flex items-center gap-1.5">
                <div className="p-0.5 bg-red-50 rounded">
                  <ServerIcon className="w-3 h-3 text-red-500" />
                </div>
                <h3 className="text-[10px] font-semibold text-slate-800">The Problem</h3>
              </div>
              <p className="text-[9px] text-slate-600 leading-relaxed">
                Distributed servers must agree on data, but networks are unreliable.
              </p>
              <div className="flex items-center gap-1.5">
                <div className="flex -space-x-1">
                  {[1, 2, 3].map((i) => (
                    <div key={i} className="w-4 h-4 rounded-full bg-slate-100 border border-white flex items-center justify-center">
                      <ServerIcon className="w-2 h-2 text-slate-400" />
                    </div>
                  ))}
                </div>
                <span className="text-[8px] text-slate-400">How do they agree?</span>
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              <div className="flex items-center gap-1.5">
                ...
                <h3 className="text-[10px] font-semibold text-slate-800">The Solution</h3>
              </div>
              <p className="text-[9px] text-slate-600 leading-relaxed">
                Paxos uses <span className="font-medium">majority voting</span>. Value commits when &gt;50% agree.
              </p>
            ...
            <div className="space-y-1.5">
              <div className="flex items-center gap-1.5">
                ...
                <h3 className="text-[10px] font-semibold text-slate-800">The Protocol</h3>
              </div>
              <div className="relative">
                <div className="absolute left-[5px] top-1.5 bottom-1.5 w-px bg-gradient-to-b from-blue-200 via-blue-300 to-blue-200" />
                <div className="space-y-1">
                  {[
                    { phase: "Prepare", desc: "Request promises" },
                    { phase: "Promise", desc: "Nodes pledge" },
                    { phase: "Accept", desc: "Broadcast value" },
                    { phase: "Consensus", desc: "Commit" },
                  ].map((step, i) => (
                    <div key={step.phase} className="flex items-center gap-1 relative">
                      <div className="w-2.5 h-2.5 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center text-[6px] font-bold border border-blue-200 z-10 bg-white">
                        {i + 1}
                      </div>
                      <span className="text-[8px] font-medium text-slate-700">{step.phase}</span>
                      <span className="text-[7px] text-slate-400">· {step.desc}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="space-y-1.5">
              <div className="flex items-center gap-1.5">
                ...
                <h3 className="text-[10px] font-semibold text-slate-800">Why It Matters</h3>
              </div>
              <div className="space-y-0.5">
                {[
                  { name: "Google Spanner", desc: "Global SQL" },
                  { name: "ZooKeeper", desc: "Coordination" },
                  { name: "etcd", desc: "Kubernetes" },
                  { name: "CockroachDB", desc: "Distributed SQL" },
                ].map((app) => (
                  <div key={app.name} className="flex items-center gap-1 text-slate-600">
                    <span className="w-1 h-1 rounded-full bg-slate-300" />
                    <span className="text-[8px] font-medium">{app.name}</span>
                    <span className="text-[7px] text-slate-400">· {app.desc}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      {/* Desktop: Full panel */}
      <div className="hidden md:block absolute bottom-0 left-0 right-0 z-30 bg-white/95 backdrop-blur-sm border-t border-slate-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                <div className="flex -space-x-1">
                  {[1, 2, 3].map((i) => (
                    <div key={i} className="w-5 h-5 rounded-full bg-slate-100 border border-white flex items-center justify-center">
            ...
            <div className="space-y-2">
              ...
              <div className="bg-slate-50 rounded px-2 py-1.5">
                ...
                <p className="text-[9px] text-slate-500 text-center mt-1">4 of 6 nodes = majority consensus</p>
              </div>
            </div>
